...
middleware.ts                 /oauth/authorize
  ...
  ├─ generate PKCE verifier      ├─ verify accounts_session
...
/auth/callback                /oauth/token
  ├─ validate state              ├─ verify client_secret (bcrypt)
  ├─ read PKCE verifier          ├─ verify code_verifier vs code_challenge
  ├─ send to backend             ├─ validate auth code (hash lookup)
  └─ set httpOnly cookies        ├─ issue access + refresh + id tokens
